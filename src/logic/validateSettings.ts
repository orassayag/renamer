...
export async function validateSettings(settings: Settings): Promise<void> {
  ...
  try {
    ...
    if (!stats.isDirectory()) {
      throw new Error(
        `Invalid settings: scanPath "${settings.scanPath}" exists but is not a directory (1000012)`
      );
    }
  } catch (error) {
  ...
  for (const ignorePath of settings.ignorePaths) {
    if (typeof ignorePath !== 'string') {
      throw new Error(
        `Invalid settings: ignorePaths must contain only strings, found: ${typeof ignorePath} (1000019)`
      );
    }

    if (ignorePath.trim().length === 0) {
      throw new Error(
        'Invalid settings: ignorePaths cannot contain empty strings (1000020)'
      );
    }

    if (ignorePath.includes('/') || ignorePath.includes('\\')) {
      throw new Error(
        `Invalid settings: ignorePaths cannot contain path separators: "${ignorePath}" (1000021)`
      );
    }
  }
  ...
  if (typeof settings.sleepAfterMilliseconds !== 'number') {
    throw new Error(
      `Invalid settings: sleepAfterMilliseconds must be a number, found: ${typeof settings.sleepAfterMilliseconds} (1000022)`
    );
  }

  if (!Number.isInteger(settings.sleepAfterMilliseconds)) {
    throw new Error(
      `Invalid settings: sleepAfterMilliseconds must be an integer, found: ${settings.sleepAfterMilliseconds} (1000023)`
    );
  }

  if (settings.sleepAfterMilliseconds < 0) {
    throw new Error(
      `Invalid settings: sleepAfterMilliseconds cannot be negative, found: ${settings.sleepAfterMilliseconds} (1000024)`
    );
  }

  if (settings.sleepAfterMilliseconds > 10000) {
    throw new Error(
      `Invalid settings: sleepAfterMilliseconds should not exceed 10000ms (10 seconds), found: ${settings.sleepAfterMilliseconds} (1000025)`
    );
  }
}
